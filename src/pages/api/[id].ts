...
async function getApiKeyByPWD(pwd: any) {
  ...
  return apiKey === null
    ? apiKey
    : "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"
}
...
export const get: APIRoute = async ({ params, request }) => {
  ...
  let _id, action, wechat, key
  try {
    //用户列表
    if (params.id === "all") {
      rslt = await listUsers()
      console.info(`all result size = ${rslt.length}`)
      return new Response(
        JSON.stringify({ code: 200, message: "success", result: rslt }),
        {
          status: 200,
          headers: {
      ...
      params.id === "del" &&
      (_id = request.headers.get("_id") || undefined) !== undefined
    ) {
      console.info(`delete user name id->${_id}`)
      rslt = await deleteUsers(_id)
      return new Response(
        JSON.stringify({ code: 200, message: "success", result: rslt }),
        {
          status: 200,
      ...
      params.id === "shell" &&
      (wechat = request.headers.get("wechatName") || undefined) !== undefined &&
      (key = request.headers.get("key") || undefined) !== undefined &&
      (action = request.headers.get("action") || undefined) !== undefined
    ) {
      const command = action === "start" ? `./webchat.sh` : `./script.sh`
      const cwd = "/usr/local/webchat/"
      const params =
        action === "start" ? [wechat, key] : ["stop", `app-${wechat}`]
      try {
        ...
        subprocess.stdout.on("data", (data: string) => {
          ...
          if (data.includes("QR") && action === "start") send(data)
        })
      ...
      return new Response(
        JSON.stringify({ code: 200, message: msg, result: rslt }),
        {
          status: 200,
          headers: {
            "Content-Type": "application/json"
      ...
      return new Response(
        JSON.stringify({ code: 400, message: "request error" }),
        {
          status: 400,
          headers: {
            "Content-Type": "application/json"
...
const checkUser = (user: {
  ...
  const regExp =
    /^\d{4}\/([1-9]|0[1-9]|1[0-2])\/([1-9]|0[1-9]|[12]\d|3[01]) ([01]\d|2[0-3]):([0-5]\d):([0-5]\d)$/
  return (
    user.name.length > 0 && user.pwd.length > 0 && regExp.test(user.expireTime)
  )
}
...
function isAuth(request: any) {
  ...
  if (request.headers.get("Authorization") !== authCode) {
    return new Response(
      JSON.stringify({ code: 401, message: "no permission" }),
      {
        status: 401,
        headers: {
          "Content-Type": "application/json"
